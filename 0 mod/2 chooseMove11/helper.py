...
def nextMoves(board, token):
    ...
    tknSet = {idx for idx in range(63) if board[idx] == getOppToken(token)} - {0, 7, 56, 63}

    for idx in tknSet: # check opposing token indexes
    ...
    #print('Token: {} possMoves {}'.format(token, possMoves))
