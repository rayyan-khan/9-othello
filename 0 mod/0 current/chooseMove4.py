...
# tried copying opening moves but no significant difference
...
EDGE_CNR = {}
for corner in CNR_EDGES:
    for edgeInd in CNR_EDGES[corner]:
        if edgeInd in EDGE_CNR:
            EDGE_CNR[edgeInd].append(corner)
        else:
            EDGE_CNR[edgeInd] = [corner]
CORNERS = {0, 7, 56, 63}
...
def sortMoves(token, oppTkn, board, possMoves):
    ...
    if board in getMove:
        sortedMoves.append(getMove[board])
        return sortedMoves
    ...
    for move in possMoves:
        ...
        if move in CORNERS:
            ...
            if board[EDGE_CNR[move][0]] == token or board[EDGE_CNR[move][1]]:
                score += 1
        if move in CX:
            if board[CX[move]] == '.':
                score = -100
...
def run(board, token):
    ...
    try:
        ...
        move = sortMoves(token, oppTkn, board, possMoves)[0]
