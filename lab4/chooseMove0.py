...
# adding tokens of given board to xTokens and oTokens
for index in range(64):
    if startboard[index] == 'x':
        xTokens.add(index)
    elif startboard[index] == 'o':
        oTokens.add(index)
...
def getScore():
    return len(xTokens), len(oTokens)
...
def checkBracketing(token, possInd, adjInd, board):
    # your token, possible placement index, adjacent index of opponent, board
...
def nextMoves(board, tokens = ''):
    ...
    if tokens == '': # if token isn't given
        token, oppToken = nextTokens(board) # assume no passes, find next token
    else:
...
def makeFlips(board, token, position):
    ...
    for opp in adjOpps: # do better later
        ...
        # maybe pass in idx rather than re-finding
        if idx:
            subset = SUBSETS[opp][position]
            changes = subset[:subset.index(idx) + 1] + [position, opp]
            board = ''.join([ch if ind not in changes else
                             token for ind, ch in enumerate(board)])
    return board


def minOppMoves(board, possMoves, token, oppToken):
    numOppMoves = {}
    # dictionary of potential moves and how many moves they'd
    # open for the opponent
    for pos in possMoves: # for each potential move
        possBrd = makeFlips(board, token, pos) # see the next board
        # there's probably a better way to do this?
        numOppMoves[pos] = nextMoves(possBrd, oppToken)[0]
    return [move for move, oppMoves
            in sorted(numOppMoves.items(), key=lambda x: x[1])]
    # return: moves by number of opposite moves yielded in ascending order
...
possMoves = minOppMoves(startboard, possMoves, startTkns, oppTkn)
...
if canMove: # if you can make a move
    ...
    # + possible moves for next opponent
        ...
        if startTkns == 'x': # if you added an x, add to set of x indexes
            xTokens.add(movePos)
        else: # otherwise add to o indexes
            oTokens.add(movePos)
