...
def corners(moveSet):
    return moveSet.intersection({0, 7, 56,  63})


def gamePhase(board):
    numMoves = 64 - board.count('.')
    if numMoves < 15:
        return 'B' # beginning
    elif numMoves < 35:
        return 'M' # middle
...
def bSortMoves(board, token, possMoves):
    sortedMoves = []
    for move in possMoves:
        flippedBoard, changes = makeFlips(board, token, move)
        sortedMoves.append((len(changes), move))
    return sorted(sortedMoves, reverse=True)

def mSortMoves(board, token, possMoves):
    sortedMoves = []
    for move in possMoves:
        flippedBoard, changes = makeFlips(board, token, move)
        if move in {0, 7, 56, 63}:
            sortedMoves.append((move, len(changes), 1))
        else:
            sortedMoves.append((move, len(changes), 0))
    return sorted(sortedMoves, key=operator.itemgetter(2))

def eSortMoves(board, token, possMoves):
    sortedMoves = []
    for move in possMoves:
        flippedBoard, changes = makeFlips(board, token, move)
        if move in {0, 7, 56, 63}:
            ...
            sortedMoves.append((move, len(changes), 0))
    return sorted(sortedMoves, key=operator.itemgetter(2), reverse=True)


# run
canMove, possMoves = nextMoves(startboard, startTkn)
oppTkn = getOppToken(startTkn)
phase = gamePhase(startboard)
if phase == 'B':
    print([tp[1] for tp in bSortMoves(startboard, startTkn, possMoves)])
elif phase == 'M':
    print([tp[1] for tp in mSortMoves(startboard, startTkn, possMoves)])
else:
    print([tp[1] for tp in eSortMoves(startboard, startTkn, possMoves)])