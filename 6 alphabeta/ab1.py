...
def estimateMoves(board, token):
    ...
    # sort from high to low so that alphabeta
    # looks at better moves first (opposite of previously)
    ...
    return [move for score, move in sorted(sortedMoves, reverse=True)]


def alphabeta(board, token, lower, upper, layer):
    # wants to return a score in [lower, upper]
    # lit: lower = alpha, upper = beta
    oppTkn = oppTkns[token]
    possMoves = nextMoves(board, token)

    if not possMoves:
        possOppMoves = nextMoves(board, token)
        if not possOppMoves:
            score = [board.count(token) - board.count(oppTkn)]
            print('Layer: {} Score: {} Token: {} Board: {} GAME OVER'
                  .format(layer, score, token, board))
            return score
        ab = alphabeta(board, oppTkn, -lower, -upper, layer + 1)
        return [-ab[0]] + ab[1:] + [-1]

    best = [lower - 1]
    for move in possMoves:
        ab = alphabeta(makeFlips(board, token, str(move), possMoves[move]),
                       oppTkn, lower, upper, layer + 1)
        score = -ab[0]
        if score > upper:
        ...
        print('Layer: {} Best: {} Board: {}'
              .format(layer, best, board))
        lower = score + 1
    return best
...
ab = alphabeta(startboard, startTkn, -65, 65, 0)
